...
import type { DateDriver } from "./date-driver";
import type { TimeUnit } from "../types";

export const jsDateDriver = (date?: Date | number): DateDriver => {
  const innerDate = date ? new Date(date) : new Date();
  const dealiasUnit = (rawUnit: string): TimeUnit => {
    if (rawUnit === "M") return "month"; // Months have an exception as they collide with minutes in their shorthand.
    const unit = rawUnit.toLowerCase();
    return match(unit)
      .with("year", "years", "y", () => "year" as const)
      .with("month", "months", () => "month" as const)
      .with("week", "weeks", "w", () => "week" as const)
      .with("date", "dates", "day", "days", "d", () => "day" as const)
      .with("hour", "hours", "h", () => "hour" as const)
      .with("minute", "minutes", "m", () => "minute" as const)
      .with("second", "seconds", "s", () => "second" as const)
      .with("month", "months", () => "month" as const)
      .otherwise(() => {
        throw new Error(`Illegal unit. Failed to parse ${rawUnit}`);
      });
  };
  ...
  return {
    startOf: function (unit: string) {
      ...
      const startOfWeek = (iso: boolean) =>
        iso ? innerDate.getDate() - innerDate.getUTCDay() : innerDate.getDate() - innerDate.getDay();
    ...
    add: function (amount: number, unit: string) {
      ...
      try {
        ...
        throw new Error(`${unit} is not recognized as a valid unit.`);
      }
    ...
    set: function (unit: string, value: number) {
      let dealiasedUnit = dealiasUnit(unit);
      if (dealiasedUnit === "week") {
        dealiasedUnit = "day";
        value = value * 7;
      }
      ...
      const method = methods[dealiasedUnit];
      innerDate[method](value);
      return this;
    },

    get: function (unit: string): number {
      let dealiasedUnit = dealiasUnit(unit);
      if (dealiasedUnit === "week") return parseInt(this.format("w"));
      ...
      try {
        // @ts-ignore // TODO
        return innerDate[methods[dealiasedUnit]]();
      } catch (e) {
        throw new Error(`${unit} is not recognized as a valid unit.`);
      }
    },
    ...
    clone: function () {
      return jsDateDriver(this.valueOf());
    },

    format: function (format: string) {
      const toOrd = (num: number) => {
      ...
      const formats = {
        ...
        MMM: () => {
          return [0, "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][
            formats.M()
          ];
        },
        MMMM: () => {
        ...
        w: () => {
          let yearStart = jsDateDriver(innerDate.valueOf()).startOf("year").valueOf();
          const weekStart = jsDateDriver(yearStart).startOf("week").valueOf();
      ...
      while (idx < format.length) {
        ...
        let lastValidToken = "";
        while (
          (currentToken && formatTokens.includes(currentToken)) ||
          (typeof currentToken === "string" && currentToken.match(/^Y{1,4}$/))
        ) {
